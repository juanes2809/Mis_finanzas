...
// ── STORAGE (localStorage para app real) ─────────────────────────────────────
function load(key) {
...
const S = {
  ...
  btn: (color = "#f2cc8f", text = "#1a1a2e") => ({
    ...
    fontWeight: 700, fontSize: "15px", background: color, color: text,
    transition: "opacity 0.15s", width: "100%", marginTop: "10px",
  ...
  pill: (active, c) => ({
    padding: "7px 14px", borderRadius: "20px", border: `2px solid ${active ? c : "rgba(255,255,255,0.12)"}`,
  ...
  modalBox: {
    ...
    padding: "24px 20px", paddingBottom: "calc(env(safe-area-inset-bottom, 20px) + 20px)",
    width: "100%", maxWidth: "520px",
    border: "1px solid rgba(255,255,255,0.1)", maxHeight: "92dvh", overflowY: "auto",
  },
};
      ...
      <div style={S.modalBox} onClick={e => e.stopPropagation()}>
        ...
        <div style={S.formGrid}>
          <div>
            ...
            <input style={S.input} type="number" inputMode="decimal" placeholder="0" value={amount} onChange={e => setAmount(e.target.value)} />
          </div>
          <div>
        ...
        <button style={S.btn(type === "expense" ? "#e07a5f" : "#52b788")} onClick={doSave}>
          Guardar {type === "expense" ? "gasto" : "ingreso"}
        </button>
        <button style={{ ...S.btn("rgba(255,255,255,0.08)", "#f0ece3"), marginTop: "8px" }} onClick={onClose}>Cancelar</button>
      </div>
    </div>
  );
}
...
function Dashboard({ txs, cats, goals }) {
  const now = new Date();
  const monthTxs = txs.filter(t => t.date.startsWith(`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}`));
  ...
  return (
    <div style={S.page}>
      <div style={{ textAlign: "center", padding: "20px 0 24px" }}>
        <div style={{ fontSize: "12px", color: "rgba(240,236,227,0.5)", textTransform: "uppercase", letterSpacing: "1px" }}>Balance del mes</div>
        <div style={{ fontSize: "36px", fontWeight: 800, marginTop: "6px", color: balance >= 0 ? "#52b788" : "#e07a5f" }}>{fmt(balance)}</div>
      </div>
      <div style={S.statGrid}>
        <div style={S.statCard("#52b788")}><div style={S.statLabel}>Ingresos</div><div style={{ ...S.statVal, color: "#52b788", fontSize: "17px" }}>{fmt(totalIncome)}</div></div>
        <div style={S.statCard("#e07a5f")}><div style={S.statLabel}>Gastos</div><div style={{ ...S.statVal, color: "#e07a5f", fontSize: "17px" }}>{fmt(totalExpense)}</div></div>
      </div>
      {goals.length > 0 && (
        <div style={S.card}>
          <div style={S.sectionTitle}>🎯 Metas del mes</div>
          {goals.map(g => {
            ...
            return (
              <div key={g.id} style={{ marginBottom: "14px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "5px" }}>
                  ...
                  <span style={{ fontSize: "12px", color: g.color, fontWeight: 700 }}>{fmt(target)}</span>
                </div>
                <div style={{ background: "rgba(255,255,255,0.08)", borderRadius: "3px", height: "6px" }}>
                  <div style={{ height: "6px", borderRadius: "3px", background: `linear-gradient(90deg, ${g.color}, ${g.color}88)`, width: "100%", transition: "width 0.6s ease" }} />
                </div>
                <div style={{ fontSize: "11px", color: "rgba(240,236,227,0.4)", marginTop: "3px" }}>
                  {g.percentage}% · {g.period === "weekly" ? "semanal" : g.period === "daily" ? "diario" : "mensual"}
                </div>
              </div>
            );
          })}
      ...
      <div style={S.card}>
        ...
        <div style={{ display: "flex", gap: "16px", marginTop: "6px" }}>
          {[["#52b788","Ingresos"],["#e07a5f","Gastos"]].map(([c,l]) => (
            <div key={l} style={{ display: "flex", alignItems: "center", gap: "5px", fontSize: "12px" }}>
              <div style={{ width: "8px", height: "8px", borderRadius: "2px", background: c }} />
              <span style={{ color: "rgba(240,236,227,0.6)" }}>{l}</span>
            </div>
          ))}
        </div>
      </div>
...
function Transactions({ txs, cats, onDelete }) {
  ...
  return (
    <div style={S.page}>
      ...
      {filtered.length === 0 && <div style={{ textAlign: "center", color: "rgba(240,236,227,0.3)", padding: "40px 0", fontSize: "14px" }}>Sin movimientos — toca + para agregar 👆</div>}
      {filtered.map(t => {
        ...
        return (
          <div key={t.id} style={S.txItem}>
            ...
            <div style={{ textAlign: "right", flexShrink: 0 }}>
              ...
              <button onClick={() => { if (confirm("¿Eliminar este movimiento?")) onDelete(t.id); }}
                style={{ background: "none", border: "none", cursor: "pointer", fontSize: "16px", marginTop: "2px" }}>🗑️</button>
            </div>
          </div>
        );
      })}
...
function dateInRange(dateStr, monday, sunday) {
...
function WeekStats({ txs, cats }) {
  ...
  const weekTxs = txs.filter(t => dateInRange(t.date, monday, sunday));
  ...
  const expMap = {};
  weekTxs.filter(t => t.type === "expense").forEach(t => { expMap[t.catId] = (expMap[t.catId] || 0) + t.amount; });
  const expPie = Object.entries(expMap).map(([id, val]) => {
    const cat = cats.find(c => c.id === id) || { name: id, icon: "❓", color: "#888" };
    return { name: `${cat.icon} ${cat.name}`, value: val, color: cat.color, pct: totalExpense > 0 ? (val/totalExpense*100).toFixed(1) : 0 };
  }).sort((a, b) => b.value - a.value);

  const incMap = {};
  weekTxs.filter(t => t.type === "income").forEach(t => { incMap[t.catId] = (incMap[t.catId] || 0) + t.amount; });
  const incPie = Object.entries(incMap).map(([id, val]) => {
    const cat = cats.find(c => c.id === id) || { name: id, icon: "❓", color: "#888" };
    return { name: `${cat.icon} ${cat.name}`, value: val, color: cat.color, pct: totalIncome > 0 ? (val/totalIncome*100).toFixed(1) : 0 };
  }).sort((a, b) => b.value - a.value);
  ...
  return (
    <div style={S.page}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "18px", background: "rgba(255,255,255,0.05)", borderRadius: "14px", padding: "12px 16px" }}>
        <button onClick={() => setWeekOffset(o => o - 1)} style={{ background: "rgba(255,255,255,0.08)", border: "none", color: "#f0ece3", borderRadius: "8px", width: "36px", height: "36px", cursor: "pointer", fontSize: "18px" }}>‹</button>
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ fontSize: "11px", color: "rgba(240,236,227,0.45)", marginTop: "2px" }}>{fmtWeekLabel(monday, sunday)}</div>
        </div>
        <button onClick={() => setWeekOffset(o => Math.min(o + 1, 0))} disabled={weekOffset === 0}
          style={{ background: weekOffset === 0 ? "rgba(255,255,255,0.03)" : "rgba(255,255,255,0.08)", border: "none", color: weekOffset === 0 ? "rgba(240,236,227,0.2)" : "#f0ece3", borderRadius: "8px", width: "36px", height: "36px", cursor: weekOffset === 0 ? "default" : "pointer", fontSize: "18px" }}>›</button>
      </div>
      ...
      <div style={S.card}>
        <div style={S.sectionTitle}>📅 Día a día</div>
        <ResponsiveContainer width="100%" height={160}>
      ...
      {expPie.length > 0 ? (
        <div style={S.card}>
          <div style={S.sectionTitle}>🔴 ¿En qué gasté?</div>
            ...
            <div style={{ flexShrink: 0 }}>
              <ResponsiveContainer width={150} height={150}>
                <PieChart>
                  ...
                  <Tooltip formatter={(v, n) => [fmt(v), n]} contentStyle={{ background: "#1a1a2e", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "8px", color: "#f0ece3", fontSize: "11px" }} />
                </PieChart>
              </ResponsiveContainer>
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              {expPie.map((d, i) => (
                <div key={i} style={{ marginBottom: "8px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "3px" }}>
                    <span style={{ fontSize: "12px", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "70%" }}>{d.name}</span>
                    <span style={{ fontSize: "12px", color: d.color, fontWeight: 700, flexShrink: 0 }}>{d.pct}%</span>
                  </div>
      ...
      {incPie.length > 0 && (
        <div style={S.card}>
          <div style={S.sectionTitle}>🟢 ¿De dónde vinieron mis ingresos?</div>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            <div style={{ flexShrink: 0 }}>
              <ResponsiveContainer width={150} height={150}>
                <PieChart>
      ...
      {weekTxs.filter(t => t.type === "expense").length > 0 && (
        <div style={S.card}>
          <div style={S.sectionTitle}>🏆 Top gastos</div>
              ...
              <div key={t.id} style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "2px" }}>
                    <span style={{ fontSize: "12px", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "65%" }}>{t.reason || cat.name}</span>
    ...
    <div style={S.page}>
      ...
      {form !== null && (
        <div style={{ ...S.card, marginTop: "14px" }}>
          ...
          <div style={S.formGrid}>
            <button style={{ ...S.btn("#f2cc8f"), marginTop: "10px" }} onClick={doSave}>Guardar</button>
      ...
      <div style={{ marginTop: "14px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div>
                  ...
                  <div style={{ fontSize: "12px", color: "rgba(240,236,227,0.45)", marginTop: "2px" }}>
                    {g.percentage}% · {g.period === "daily" ? "diario" : g.period === "weekly" ? "semanal" : "mensual"} · {sourceLabel}
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
                  ...
                  <div style={{ display: "flex", gap: "6px", marginTop: "8px", justifyContent: "flex-end" }}>
                    ...
                    <button style={S.btnSm("#e07a5f22", "#e07a5f")} onClick={() => { if(confirm("¿Eliminar meta?")) onDelete(g.id); }}>🗑️</button>
                  </div>
                </div>
              </div>
            </div>
        ...
        {goals.length === 0 && <div style={{ textAlign: "center", color: "rgba(240,236,227,0.3)", padding: "30px 0", fontSize: "14px" }}>No tienes metas — ¡crea una! 🎯</div>}
      </div>
    </div>
  );
}
...
function Categories({ cats, onSave, onDelete }) {
  ...
  const CatList = ({ list }) => list.map(c => (
    <div key={c.id} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 12px", borderRadius: "10px", background: "rgba(255,255,255,0.04)", marginBottom: "6px" }}>
      ...
      <button style={S.btnSm("#e07a5f22", "#e07a5f")} onClick={() => { if(confirm("¿Eliminar?")) onDelete(c.id); }}>🗑️</button>
    </div>
  ));
  return (
    <div style={S.page}>
        ...
        <div style={{ ...S.card, marginTop: "14px" }}>
          ...
          <div style={{ marginTop: "10px" }}>
            ...
            <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
              {COLORS.map(c => (
                <div key={c} onClick={() => setForm(p => ({ ...p, color: c }))} style={{ width: "28px", height: "28px", borderRadius: "50%", background: c, cursor: "pointer", border: F.color === c ? "3px solid white" : "3px solid transparent" }} />
              ))}
            </div>
          </div>
          <div style={S.formGrid}>
